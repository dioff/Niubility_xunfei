  ...
  /***离线命令词识别服务参数设置***/
	...
	while(ros::ok())
	{
		if(awake_flag)    //判断休眠状态还是唤醒状态
		{
			...
			{
				ROS_INFO("[call_rec] succeed to call service \"get_offline_recognise_result_srv\"!");    //打印识别结果、置信度、识别命令词等信息
				std::cout << "[call_rec] result: " << GetOfflineResult_srv.response.result << endl;
				std::cout << "[call_rec] fail reason: " << GetOfflineResult_srv.response.fail_reason << endl;
				std::cout << "[call_rec] text: " << GetOfflineResult_srv.response.text << endl;
				...
				else if(str2 == GetOfflineResult_srv.response.result)    //记录识别失败次数
				{
					...
					printf("[call_rec] recognize_fail_count=%d\n",recognize_fail_count);
			...
			{
				ROS_INFO("[call_rec] failed to call service \"get_offline_recognise_result_srv\"!");
				//awake_flag=0;
				break;
			}
