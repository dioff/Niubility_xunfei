...
char *join(std::string b, char *s2)
{
	...
	{
		cout << "[voice_control] -->>>>>join拷贝失败" << endl;
	}
...
int business_proc_callback(business_msg_t businessMsg)
{
	...
	case 0x01:
		...
		else if (businessMsg.msgId == 0x02)
		{
			...
			{
				cout << "[voice_control] -->>>>>拷贝失败" << endl;
			}
			if (businessMsg.length < len)
			{
				len = businessMsg.length;
				cout << "[voice_control] -- businessMsg size is noenough" << endl;
			}
			if (save_pcm_local)
			{
			...
			{
#if whether_print_log
				printf("[voice_control]***************write the first voice**********\n");
#endif
			...
			{
#if whether_print_log
				printf("[voice_control]***************write the middle voice**********\n");
#endif
		...
		else if (businessMsg.msgId == 0x04)
		{
			...
			if (status == 0)
			{
				printf("[voice_control] -->>>>>开/关原始音频成功\n");
			}
		}
		...
		else if (businessMsg.msgId == 0x07)
		{
			...
			try
			{
				...
				printf("[voice_control] -->>>>>主麦克风id为%d号麦克风\n", major_mic_id);
			}
			catch (...)
			{
	...
	case 0x02:
		if (businessMsg.msgId == 0x01)
		{
			...
			if (major_mic_id <= 5 && major_mic_id >= 0 && mic_angle <= 360 && mic_angle >= 0)
			{
				...
				if (ret1 == 0 && ret2 == 0)
				{
					printf("[voice_control] -->>>>>第%d个麦克风被唤醒\n", major_mic_id);
					printf("[voice_control] -->>>>>唤醒角度为:%d\n", mic_angle);
					printf("[voice_control] -->>>>>已点亮%d灯\n", led_id);
	...
	case 0x04:
		...
		{
			printf("[voice_control] config.json: %s", businessMsg.data);
		}
		break;
...
*/
		...
		{
#if whether_print_log
			printf("[voice_control] --[01]创建语音识别引擎失败！\n");
#endif
		}

		printf("[voice_control] -->>>>>开始一次语音识别！\n");
		...
		while (whether_finised != 1)
		{
			...
			{
				cout << "[voice_control] 超出离线命令词最长识别时间\n"
					 << endl;
				break;
			}
		}
		...
		if (whole_result!="")
		{
			...
			{
				printf("[voice_control] -->>>>>是否识别成功:　[ %s ]\n", "是");
				printf("[voice_control] -->>>>>关键字的置信度: [ %d ]\n", effective_ans.effective_confidence);
				printf("[voice_control] -->>>>>关键字识别结果: [ %s ]\n", effective_ans.effective_word);
				/*发布结果*/
			...
			else
			{
				printf("[voice_control] -->>>>>是否识别成功:　[ %s ]\n", "否");
				printf("[voice_control] -->>>>>关键字的置信度: [ %d ]\n", effective_ans.effective_confidence);
				printf("[voice_control] -->>>>>关键字置信度较低，文本不予显示\n");
				res.result = "fail";
		...
		else
		{
			...
			res.text = " ";
			printf("[voice_control] -->>>>>未能检测到有效声音,请重试\n");
		}
		whole_result = "";
		/*[1-3]语音识别结束]*/
		delete_asr_engine();
...
bool Record_Start(xf_mic_asr_offline::Start_Record_srv::Request &req, xf_mic_asr_offline::Start_Record_srv::Response &res)
{
	...
	{
		ROS_INFO("[voice_control] got topic request,start to record ...\n");
	...
	{
		ROS_INFO("[voice_control] got topic request,stop to record ...\n");
...
{
	ROS_INFO("[voice_control] got request,start to correct awake word ...\n");
	...
	{
		ROS_INFO("[voice_control] got request,stop to correct awake word...\n");
		res.result = "fail";
...
{
	ROS_INFO("[voice_control] got topic request,start to make the target led on ...\n");
	char str[256] = {0};
				   ...
				   xf_mic_asr_offline::Get_Major_Mic_srv::Response &res)
{
	...
	{
		ROS_INFO("[voice_control] got request,start to get the major mic id ...\n");
		Get_request_mic_id = false;
		get_major_mic_id();
		clock_t startTime, endTime;
		startTime = clock(); //计时开始
					 ...
					 xf_mic_asr_offline::Get_Awake_Angle_srv::Response &res)
{
	...
	{
		ROS_INFO("[voice_control] got request,start to get the major awake angle ...\n");
		clock_t startTime, endTime;
		startTime = clock(); //计时开始
...
int main(int argc, char *argv[])
{
	...
	ndHandle.param("source_path", source_path, std::string("/home/EPRobot/robot_ws/src/xf_mic_asr_offline/"));
	ndHandle.param("/appid", appid, std::string("5fa0b8b9"));//appid，需要更换为自己的

	printf("-----confidence =%d\n",confidence);
	printf("-----time_per_order =%d\n",time_per_order);

	cout<<"[voice_control] source_path="<<source_path<<endl;
	cout<<"[voice_control] appid="<<appid<<endl;
	...
	if (!handle)
	{
		printf("[voice_control] -->>>>>无法打开麦克风设备，尝试重新连接进行测试\n");
		return -1;
	}
	printf("[voice_control] -->>>>>成功打开麦克风设备\n");
	...
	{
		printf("[voice_control] -->>>>>未设置主麦，请唤醒或设置主麦\n");
	}
	while (major_mic_id>5 || major_mic_id<0)
	{
		sleep(1);
	}
	printf("[voice_control] -->>>>>设置主麦成功！\n");
