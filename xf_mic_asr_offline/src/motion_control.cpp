...
void shutdown(int sig)
{
	...
	ROS_INFO("[motion_ctrl] motion_control.cpp ended!");
	ros::shutdown();
}
...
void follow_turn(int angle)
{
	...
	printf("[motion_ctrl] time =%f\n",angular_duration);
	ticks = int(angular_duration * rate1);    //计算控制次数
	printf("[motion_ctrl] ticks =%d\n",ticks);
...
void akm_follow_turn(int angle)
{
	...
	try{
        ...
        */
                ...
                ROS_ERROR("[motion_ctrl] Received an exception trying to transform a point from \"laser\" to \"base_link\": %s", ex.what());
        }

//fangxiang 
	goal_angle_r=(direction-angle)/180*3.1415926;
	printf("[motion_ctrl] goal_angle_r=%f\n",goal_angle_r);
	...
	cout<<"[motion_ctrl] reach: "<<reach<<endl;
	//std_msgs::Int8 msg;
   // msg.data= 0;
    //goal_reach_pub.publish(msg);
    //printf("666666666666666\n");
	reach = "";
	printf("[motion_ctrl] 1111111111222222222222\n");
	cout<<"[motion_ctrl] reach: "<<reach<<endl;
}
...
int main(int argc, char** argv)
{
  ...
  /***创建障碍物方位话题订阅者***/
	...
	ROS_INFO("[motion_ctrl] motion_control.cpp start...");
